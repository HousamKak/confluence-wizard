...
async function fetchAllConfluenceData(spaceKey) {
  ...
  while (hasMoreData) {
    const response = await api.asApp().requestConfluence(route`/wiki/rest/api/content?spaceKey=${spaceKey}&start=${start}&limit=${limit}`);
...
resolver.define('question_to_gpt', async ({ payload }) => {
  ...
  try {
    ...
    const params = {
      model: "gpt-3.5-turbo",
      messages: [
        { role: "system", content: "You are an expert in the knowledge base that I provide you with. You have to answer any question that I ask you strictly from the information available in the knowledge base I provide. You shall not answer anything unethical at all. Limit all your answers to the knowledge base. Information outside the knowledge base are prohibited" },
    ...
    if (data.choices && data.choices[0] && data.choices[0].message.content) {
      ...
      logger.info('Answer generated by GPT-3.5:', answer);
      console.log('Answer generated by GPT-3.5:', answer);
      return { answer: answer, status_code: 200 };
    } else {
      throw new Error('GPT-3.5 did not return a valid response.');
    }
  } catch (error) {
    logger.error(`Failed to get answer from GPT-3.5: ${error.message}`);
    console.error(`Failed to get answer from GPT-3.5: ${error.message}`);
    return { answer: `Failed to get answer from GPT-3.5: ${error.message}`, status_code: 500 };
  }
});
